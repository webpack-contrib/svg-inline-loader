      ...
      if (attrs.length > 0) {
        ...
        console.warn(
          `svg-inline-loader: tag ${
            tag.tagName
          } has forbidden attrs: ${attrList.join(', ')}`
        );
      }
    }
    return tag;
...
function createIdPrefix(idPrefix) {
  ...
  return function prefixIds(tag) {
    ...
    if (tag.tagName === 'use') {
      ...
      if (hrefIdx !== -1) {
        tag.attributes[hrefIdx][1] = `#${idPrefix}${tag.attributes[
          hrefIdx
        ][1].substring(1)}`;
      }
    }
...
function runTransform(tokens, configOverride) {
  ...
  const config = conditions.isFilledObject(configOverride)
    ? Object.assign({}, defaultConfig, configOverride)
    : defaultConfig;

  if (config.classPrefix !== false)
    transformations.push(createClassPrefix(config.classPrefix));
  if (config.idPrefix !== false)
    transformations.push(createIdPrefix(config.idPrefix));
  if (config.removeSVGTagAttrs === true)
    transformations.push(removeSVGTagAttrs);
  if (config.warnTags.length > 0)
    transformations.push(createWarnTags(config.warnTags));
  if (config.removeTags === true)
    transformations.push(createRemoveTags(config.removingTags));
  if (config.warnTagAttrs.length > 0)
    transformations.push(createWarnTagAttrs(config.warnTagAttrs));
  if (config.removingTagAttrs.length > 0)
    transformations.push(createRemoveTagAttrs(config.removingTagAttrs));
