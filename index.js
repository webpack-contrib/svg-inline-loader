...
function getExtractedSVG(svgStr, options) {
  ...
  if (!!options) {
    ...
    if (!!config.classPrefix) {
      const name =
        config.classPrefix === true ? '__[hash:base64:7]__' : config.classPrefix
      config.classPrefix = loaderUtils.interpolateName({}, name, {
        content: svgStr
      })
    }
    ...
    if (!!config.idPrefix) {
      ...
      config.idPrefix = loaderUtils.interpolateName({}, id_name, {
        content: svgStr
      })
    }
  }
  ...
  var tokens
  var cleanedUp = regexSequences
    .reduce(function(prev, regexSequence) {
...
function SVGInlineLoader(content) {
  ...
  if (options.esModules) {
    return 'export default ' + JSON.stringify(getExtractedSVG(content, options))
  }
